            ...
            >
              <div className="flex flex-wrap items-start justify-between gap-4">
                <div className="space-y-1">
                  <p className="text-dark-text font-medium">{o.product_name || 'Unknown Product'}</p>
                  <p className="text-dark-muted text-sm">Qty: {o.quantity}</p>
                  <p className="text-dark-muted text-sm">{o.customer_name} &middot; {o.phone}</p>
                  {o.note && <p className="text-dark-muted text-xs italic">"{o.note}"</p>}
                  <p className="text-dark-muted text-xs">{new Date(o.created_at).toLocaleDateString()}</p>
                </div>
                <div className="flex items-center gap-3">
                  <span className={`px-3 py-1 rounded-full text-xs font-medium ${o.status === 'processed' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-amber-500/20 text-amber-400'}`}>
